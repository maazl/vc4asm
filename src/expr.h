...
,	R_RDB  =  6 ///< read from register file A
,	R_RDAB =  7 ///< read from register file A
,	R_WRA  =  9 ///< read from register file A
,	R_WRB  = 10 ///< read from register file A
,	R_WRAB = 11 ///< read from register file A
...
,	R_SREL = 16 ///< semaphore release [Value must not change!!!]
,	R_SACQ = 32 ///< semaphore acquire
,	R_SEMA = 48 ///< any semaphore type
};
struct reg_t
{	uint8_t     Num;   ///< register number
	regType     Type;  ///< register type
	uint8_t     Rotate;///< QPU element rotation [0..15]
};
enum valueType : char
{	V_NONE      ///< no value
